...
def get_dataset(name, train=True, permutation=None, capacity=None, label=None):
...
TRAIN_DATASETS = {
    'fashion_mnist': lambda: datasets.FashionMNIST(
    ...
    'cifar10': lambda: datasets.CIFAR10(
        ...
        # target_transform=transforms.Compose(_CIFAR_TARGET_TRANSFORMS)
    ),
...
TEST_DATASETS = {
    'fashion_mnist': lambda: datasets.FashionMNIST(
        './datasets/fashion_mnist', train=False, download=True,
        transform=transforms.Compose(_MNIST_TRAIN_TRANSFORMS)
    ),
    'mnist': lambda: datasets.MNIST(
...
DATASET_CONFIGS = {
    ...
    'fashion_mnist': {'size': 32, 'channels': 1, 'classes': 10},
