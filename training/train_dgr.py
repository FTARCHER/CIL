...
def _generator_training_callback(
    ...
    if vis_show:
        ...
        # dgr_visual.visualization.replay_log('./CL_result_test')
...
def _solver_training_callback(
    ...
    def cb(solver, progress, batch_index, result):
        ...
        if iteration % eval_log_interval == 0:
            ...
            over_accuracy = sum(accuracies)/current_task
            print('over_accuracy=',over_accuracy)
            print('sum(accuracies)=',sum(accuracies))
            ...
            #             testdataset = None
            #             for j in range(i):
            #                 testdataset = ConcatDataset([testdataset,test_datasets[j]])
            #             dgr_utils.validate(
            #                     solver, testdataset, test_size=test_size,
            #                     cuda=cuda, verbose=False, collate_fn=collate_fn
            #                 )
            #         else:
            #             pass
            ...
            #             else:
            #                 testdataset = ConcatDataset([testdataset,test_datasets[j]])
            #         accuracies.append(dgr_utils.validate(
            #                     solver, testdataset, test_size=len(testdataset),
            #                     cuda=cuda, verbose=False, collate_fn=collate_fn
            #                 ))
            #     else:
            #         accuracies.append(0)
