...
class Solver(BatchTrainable):
    ...
    def train_a_batch(self, x, y, x_=None, y_=None, importance_of_new_task=.5):
        ...
        # ftnum, real_predicted = real_scores.max(1)
        # print(fttnum)
        ...
        if x_ is not None and y_ is not None:
            ...
            # precision = importance_of_new_task * real_prec+(1-importance_of_new_task) * replay_prec
        ...
        with open('./results/log.txt', 'a+') as log:
            log.write(f'Classifier loss: {loss.item()}')
...
class Scholar(GenerativeMixin, nn.Module):
    ...
    def train_with_replay(
            ...
            importance_of_new_task=None, batch_size=32,
    ...
    def _train_batch_trainable_with_replay(
        ...
        # create data loaders.
        data_loader = iter(dgr_utils.get_data_loader(
            dataset, batch_size, cuda=self._is_on_cuda(),
            collate_fn=collate_fn,
        ))
        data_loader_previous = iter(dgr_utils.get_data_loader(
